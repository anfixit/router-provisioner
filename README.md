...
Обслуживание идёт ночью, чтобы не трогать работающие соединения днём:

| Время | Что происходит |
| --- | --- |
| 03:00 | обновление подписки с автоматическим откатом: если после обновления sing-box не поднимается, возвращается предыдущая рабочая конфигурация |
| 03:30 | обновление списков маршрутизации |
| 03:45 | обновление sing-box, если вышла новая сборка |
| каждую минуту | проверка закреплённых узлов, возврат сервиса на основной узел |

Обновление sing-box стоит отдельного слова. Отставшая версия ничем себя не
выдаёт: NetShift показывает зелёные проверки, а подписка выглядит мёртвой —
на роутере отвечают единицы узлов, тогда как с телефона по той же подписке
работают все. Ночной хелпер сверяет версию сам и ставит новую сборку, а если
обновлять нечего — не перезапускает NetShift, чтобы не сбрасывать закреплённые
узлы впустую.

Сам NetShift обновляется вручную: его установщик задаёт вопросы и может сбросить
настройки. Когда выходит новая версия, скрипт пишет об этом в журнал.
